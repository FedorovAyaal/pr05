    ...
    <div class="container mx-auto w-4/5 px-5 ">
        <div class="flex items-center justify-between flex-wrap py-6">
            ...
            <div class="flex items-center flex-shrink-0 text-white mr-6">
              ...
              <span class="font-semibold text-xl tracking-tight"><a href="{{route('home')}}">News Sakha<a></span>
            </div>
            ...
            <div class="w-full hidden flex-grow lg:flex lg:items-center lg:w-auto" id="menu">
              <div class="text-sm lg:flex-grow">
                ...
                <a href="#" class="block mt-4 lg:inline-block lg:mt-0 text-teal-100 hover:underline hover:underline-offset-4 hover:text-white mr-4">
                  {{$category->name}}
                </a>
                @endforeach
              </div>
              <div>
                ...
                @if(auth()->check())
                @can('view',auth()->user())
                <a href="{{route('post.create')}}" class="inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0">Admin</a>
                @endcan
                <form action="{{route('logout')}}" method="POST" class="inline-block">
                    @csrf
                    <button class="inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0" type="submit">Выйти</button>
                </form>
