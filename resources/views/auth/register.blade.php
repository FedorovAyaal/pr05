...
<form action="{{route('register')}}" method="POST">
    ...
    <div class="w-3/4 lg:w-1/2 mx-auto mt-10">
        <h1 class="text-3xl mb-10">Регистрация</h1>
        ...
        <div class="relative z-0 mb-6 w-full group">
            <input type="text" name="name" id="floating_email" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer focus:bg-none" placeholder=" " required value="{{old('name')}}"/>
            @error('name')
            <p class="text-red-500"><small>{{$message}}</small></p>
            @enderror
        ...
        <div class="grid md:grid-cols-2 md:gap-6">
          <div class="relative z-0 mb-6 w-full group">
              <input type="text" name="first_name" id="floating_first_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required  value="{{old('first_name')}} "/>
              @error('first_name')
              <p class="text-red-500"><small>{{$message}}</small></p>
              @enderror
          ...
          <div class="relative z-0 mb-6 w-full group">
              <input type="text" name="last_name" id="floating_last_name" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required  value="{{old('last_name')}} "/>
              @error('last_name')
              <p class="text-red-500"><small>{{$message}}</small></p>
              @enderror
        ...
        <p class="mt-2"><small>Уже есть аккаунт? <a href="{{route('login')}}" class="text-green-400 hover:text-blue-700 font-bold">Войти</a></small></p>
      </div>
</form>
